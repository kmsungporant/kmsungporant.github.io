...
import {
  SiCplusplus,
  SiCss3,
  SiDocker, SiFramer, SiGithubactions,
  SiGooglecloud,
  SiHtml5,
  SiJava,
  SiJavascript,
...
const icons: { name: string; icon: ReactNode; link: string }[] = [
  ...
  { name: "Google Cloud", icon: <SiGooglecloud />, link: "https://cloud.google.com/" },
...
export default function Icons() {
  return (
    <div className="flex flex-col items-center mt-16">
      <motion.h1 className="p-5 text-4xl font-black underline text-dark-secondary dark:text-light-secondary"
        initial={{ opacity: 0 }}
        whileInView={{
          opacity: 1,
          transition: {
      ...
      <div className="grid grid-cols-5 lg:grid-cols-6 place-self-center xl:flex">
        {icons.map((icon, i) => (
          <Link key={i} passHref href={icon.link}>
            <motion.a
              target="_blank"
              className="m-2 text-5xl text-dark-secondary hover:text-dark-primary dark:text-light-secondary hover:dark:text-light-primary"
              initial={{ opacity: 0 }}
              whileInView={{
                opacity: 1,
                transition: {
